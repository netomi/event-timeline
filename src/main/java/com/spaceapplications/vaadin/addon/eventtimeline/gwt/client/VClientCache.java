...
/**
 * Client-side cache for received events, based on original version from vaadin-timeline.
 * 
...
public class VClientCache {
   ...
   *          THe events
   */
  public void addToCache(Integer band, Date from, Date to,
      List<VEvent> events) {
   ...
   * @param to
   *          The end date
   * @return Returns a list of events. If the range was not found in cache null
   *         is returned.
   */
      ...
      if ((dr.inRange(from) && dr.inRange(to))
        ...
        if (dr.getEvents() != null) {
          for (VEvent event : dr.getEvents()) {
            ...
            if (time >= dr.getStartTime() && time <= dr.getEndTime()) {
              events.add(event);
            }
          }
        }
