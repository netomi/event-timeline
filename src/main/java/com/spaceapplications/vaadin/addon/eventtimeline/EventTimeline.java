...
public class EventTimeline extends AbstractComponent implements
	...
	/*
	 * We need to keep count of the date range listeners so we can turn off the
	...
	static {
		try {
			...
			throw new java.lang.RuntimeException(
					"Internal error finding methods in EventTimeline");
		}
	}
		...
		if (sendBands) {
			if (bandsToBeRemoved.size() > 0 || bandsToBeAdded.size() > 0) {
				...
				for (BandInfo band : bandsToBeAdded) {
					BandsPainter.paintAdd(target, band.getBandId(),
							band.getCaption());
				}
	...
	/**
	 * Paints single timeline event to UIDL. Override this method to add custom
	 ...
	 *            PaintTarget
	 */
	protected void paintEvent(TimelineEvent e, PaintTarget target)
		...
		target.addAttribute(VEventTimelineWidget.ATTR_DATEFROM,
				df_date.format(e.getStart()));
		target.addAttribute(VEventTimelineWidget.ATTR_DATETO,
				df_date.format(e.getEnd()));
		target.addAttribute(VEventTimelineWidget.ATTR_TIMEFROM, e.getStart()
				.getTime());
		target.addAttribute(VEventTimelineWidget.ATTR_TIMETO, e.getEnd()
				.getTime());
	...
	protected String[] getEventsAsStringArray(final List<TimelineEvent> events)
			throws PaintException {
	 ...
	 * 
	 * @param color
	 *            The color (as CSS3-style rgba string) of the grid or Null to
	 *            remove the grid
	 */
