...
public class VEventTimelineWidget extends Composite implements Paintable {
  ...
  private void recalculateHeights() {
    ...
    int displayHeight =
      getWidgetHeight() - captionHeight - topBarHeight - modeLegendBarHeight - browserHeight;
  ...
  private Timer refreshTimer = new Timer() {
    ...
    public void run() {
      ...
      if (selectionLock) {
        /*
         * Update the selection manually. If the selection lock is not on the browser will update
         * the display once it has finished loading and recalculated the new selection
         */
  ...
  private void setDateSelectVisibility(UIDL uidl) {
    if (uidl.hasAttribute("dateSelectVisibility")) {
      dateSelectVisible = uidl.getBooleanAttribute("dateSelectVisibility");
  ...
  private void setBandCaption(UIDL uidl) {
    if (uidl.hasAttribute("numBands")) {
      numBands = uidl.getIntAttribute("numBands");
    }

    if (uidl.hasAttribute("bcaption")) {
      String[] captions = uidl.getStringArrayAttribute("bcaption");
      bandArea.setCaptions(captions);
    }
   ...
   * (non-Javadoc)
   * 
   * @see com.vaadin.terminal.gwt.client.Paintable#updateFromUIDL(com.vaadin.terminal
   ...
   */
  public boolean getEvents(VDataListener component, Date startDate, Date endDate, boolean useCache) {
    ...
    GWT.log("trying to retrieve events: start=" + startDate.toString() + ", end="
        + endDate.toString());

    if (useCache) {
      boolean gotFromCache = true;
      for (int band = 0; band < numBands; band++) {
        List<VEvent> events = cache.getFromCache(band, startDate, endDate);
        if (events == null) {
    ...
    client.updateVariable(uidlId, "events",
        new Object[] { Long.valueOf(startDate.getTime()), Long.valueOf(endDate.getTime()) }, false);
  ...
  private void zoomLevelClicked(ClickEvent evt) {
    ...
    if (totalTime >= time) {
      ...
      if (browserIsVisible) {
        Long selectedTime =
          browser.getSelectedEndDate().getTime() - browser.getSelectedStartDate().getTime();
        center = new Date(browser.getSelectedStartDate().getTime() + selectedTime / 2L);
      } else {
        center = new Date(display.getSelectionStartDate().getTime() + time / 2L);
      }
  ...
  public void fireDateRangeChangedEvent() {
    if (display.getSelectionStartDate() != null && display.getSelectionEndDate() != null) {
      Object[] values =
        new Object[] { display.getSelectionStartDate().getTime(),
  ...
  public int getWidgetHeight() {
    try {
      int height =
        Integer.parseInt(DOM.getAttribute(root.getElement(), "height").replaceAll("px", ""));
      return height;
    } catch (Exception e) {
      try {
        int height =
          Integer.parseInt(DOM.getStyleAttribute(root.getElement(), "height").replaceAll("px", ""));
        return height;
      } catch (Exception f) {
  ...
  public int getWidgetWidth() {
    try {
      int width =
        Integer.parseInt(DOM.getAttribute(root.getElement(), "width").replaceAll("px", ""));
      return width;
    } catch (Exception e) {
      try {
        int width =
          Integer.parseInt(DOM.getStyleAttribute(root.getElement(), "width").replaceAll("px", ""));
        return width;
      } catch (Exception f) {
  ...
  private void setDateFormatInfo(UIDL uidl) {
    if (uidl.hasAttribute("dateformats")) {
      ...
      DateTimeFormat dateSelectDisplaySimpleDateFormat = DateTimeFormat.getFormat(formats[0]);
      setSelectDisplayFormat(dateSelectDisplaySimpleDateFormat);

      DateTimeFormat dateSelectEditSimpleDateFormat = DateTimeFormat.getFormat(formats[1]);
      setSelectEditFormat(dateSelectEditSimpleDateFormat);

      DateTimeFormat shortYearFormat = DateTimeFormat.getFormat(formats[2]);
      display.setYearFormatShort(shortYearFormat);

      DateTimeFormat longYearFormat = DateTimeFormat.getFormat(formats[3]);
      display.setYearFormatLong(longYearFormat);

      DateTimeFormat shortMonthFormat = DateTimeFormat.getFormat(formats[4]);
      display.setMonthFormatShort(shortMonthFormat);

      DateTimeFormat longMonthFormat = DateTimeFormat.getFormat(formats[5]);
      display.setMonthFormatLong(longMonthFormat);

      DateTimeFormat shortDayFormat = DateTimeFormat.getFormat(formats[6]);
      ...
      DateTimeFormat shortTimeFormat = DateTimeFormat.getFormat(formats[8]);
      display.setTimeFormatShort(shortTimeFormat);

      DateTimeFormat longTimeFormat = DateTimeFormat.getFormat(formats[9]);
